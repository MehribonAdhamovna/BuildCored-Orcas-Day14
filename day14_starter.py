...
def execute(cpu, instruction):
    ...
    # MOV Rx, <value> or MOV Rx, Ry
    if op == "MOV":
        dst = reg_index(args[0])
        val = get_value(cpu, args[1])
        cpu.registers[dst] = val
        cpu.pc += 1
        return f"Moved {val} into R{dst}"

    # Arithmetic: ADD, SUB, MUL
    elif op in ("ADD", "SUB", "MUL"):
        dst = reg_index(args[0])
        val_b = get_value(cpu, args[1])
        val_a = cpu.registers[dst]
        
        # Use the pre-built ALU
        result = alu(op, val_a, val_b)
        cpu.registers[dst] = result
        cpu.pc += 1
        return f"ALU {op}: {val_a} and {val_b} -> R{dst} ({result})"

    # CMP Rx, Ry (Sets flags for jumping)
    elif op == "CMP":
        val_a = get_value(cpu, args[0])
        val_b = get_value(cpu, args[1])
        cpu.flags["ZERO"] = (val_a == val_b)
        cpu.flags["NEG"] = (val_a < val_b)
        cpu.pc += 1
        return f"Compared {val_a} and {val_b} (Zero: {cpu.flags['ZERO']})"

    # Control Flow: JMP, JZ, JNZ
    elif op == "JMP":
        cpu.pc = int(args[0])
        return f"Unconditional jump to line {cpu.pc}"

    elif op == "JZ":
        if cpu.flags["ZERO"]:
            cpu.pc = int(args[0])
            return f"Zero flag set: jumping to line {cpu.pc}"
        cpu.pc += 1
        return "Zero flag clear: no jump"

    elif op == "JNZ":
        if not cpu.flags["ZERO"]:
            cpu.pc = int(args[0])
            return f"Zero flag clear: jumping to line {cpu.pc}"
        cpu.pc += 1
...
NARRATOR_PROMPT = """You are a Computer Architecture Professor. 
Explain the following CPU transition in one concise, punchy sentence using technical hardware terminology.

Instruction: {instr}
Action Taken: {effect}
Register State: {regs}

Focus on the interaction between the Registers, the ALU, and the Program Counter. 
Do not say "Sure" or "Here is the explanation." Just the sentence."""
